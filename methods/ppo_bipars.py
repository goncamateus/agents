...
class PPOBipars(nn.Module):
    ...
    def get_z(self, x):
        z_t = self.z(x) + 1
        z_t = torch.max(torch.ones_like(z_t), z_t)
        return z_t
    ...
    def update(self, clipfracs, batch):
        ...
        z_loss1 = ratio * mb_advantages
        z_loss2 = (
            torch.clamp(ratio, 1 - ratio_clip_param_f, 1 + ratio_clip_param_f)
            * mb_advantages
        )
        z_loss = -torch.min(z_loss1, z_loss2).mean()
        ...
        return old_approx_kl, approx_kl, v_loss, pg_loss, entropy_loss, z_loss
